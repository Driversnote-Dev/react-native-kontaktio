...
import type { EmitterSubscription, ColorValue } from 'react-native';
...
type State = {
  ...
  ranging: boolean;
  monitoring: boolean;
  discoveredBeacons: Array<IBeaconIosDiscoveryWithMajorMinor>;
  rangedBeacons: Array<IBeaconIos>;
  rangedRegions: Array<RegionType>;
  monitoredRegions: Array<RegionType>;
  monitoredRegionsCloseBy: Array<RegionType>;
...
export default class IBeaconExample extends Component<{}, State> {
  ...
  startMonitoringSubscription: EmitterSubscription | null = null;
  monitoringFailSubscription: EmitterSubscription | null = null;
  regionEnterSubscription: EmitterSubscription | null = null;
  regionExitSubscription: EmitterSubscription | null = null;
  regionRangeSubscription: EmitterSubscription | null = null;
  regionRangeFailSubscription: EmitterSubscription | null = null;
  authorizationSubscription: EmitterSubscription | null = null;
  discoverSubscription: EmitterSubscription | null = null;
  discoverFailSubscription: EmitterSubscription | null = null;
   ...
   */
  _isIdenticalRegion = (r1: RegionType, r2: RegionType) =>
  ...
  _renderDiscoveredBeacons = () => {
    ...
    return this.state.discoveredBeacons
      .sort(
        (
          a: IBeaconIosDiscoveryWithMajorMinor,
          b: IBeaconIosDiscoveryWithMajorMinor
        ) => a.rssi - b.rssi
      )
      .map(
        (beacon: IBeaconIosDiscoveryWithMajorMinor, index: number) => (
          <View
            key={index}
            style={[
              styles.beacon,
  ...
  _renderRangedBeacons = () => {
    ...
    return this.state.rangedBeacons
      .sort(
        (a: IBeaconIos, b: IBeaconIos) =>
          parseInt(a.accuracy) - parseInt(b.accuracy)
      )
      .map(
        (beacon: IBeaconIos, index: number) => (
          <View
            key={index}
            style={[
              styles.beacon,
  ...
  _renderMonitoredRegions = () => {
    ...
    return this.state.monitoredRegionsCloseBy.map(
      (region: RegionType, index: number) => (
        <View
          key={index}
          style={[
            styles.beacon,
